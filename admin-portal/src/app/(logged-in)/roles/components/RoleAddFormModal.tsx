...
import { AddFormKey } from "../types/RoleTypes";
...
const RoleAddFormModal: React.FC<RoleFormModalProps> = ({
  isOpen,
  onClose,
  onRoleAdded,
}) => {
  const { permissionList } = useGetPermissionList();
  const { form, isPending } = useAddRole(() => {
    onRoleAdded();
    onClose();
    form.resetForm();
  });
  ...
  const togglePermission = (id: number) => {
    ...
    const updated = selected.includes(id)
      ...
      : [...selected, id];
    form.update("permissions" as AddFormKey, updated);
  };
  ...
  const toggleSelectAll = (ids: number[], checked: boolean) => {
    ...
    form.update("permissions" as AddFormKey, Array.from(selected));
  };
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5">Define role name and assign permissions</p>
                </div>
              </div>
              <button
                onClick={handleClose}
            ...
            <form action={form.action} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    form.update("name" as AddFormKey, e.target.value)
                  }
                  onBlur={() => form.onBlur("name" as AddFormKey)}
                  required
                ...
                />
                {form.shouldShowError("name" as AddFormKey) && (
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <label className="text-xs font-medium flex items-center gap-2 cursor-pointer hover:text-blue-600 transition-colors">
                              <input
                                type="checkbox"
                                checked={allChecked}
                                onChange={(e) => toggleSelectAll(ids, e.target.checked)}
