...
export default function Address({ fields }: { fields: any[] }) {
  const { divisions, districts, thanas, postal_codes } = useAddress();
  const getValue = (field: any) => {
    if (field.slug.includes("division")) {
      return (
        divisions.find((option: any) => option.value === field.value)?.label ||
        ""
      );
    }
    if (field.slug.includes("district")) {
      return (
        districts.find((option: any) => option.value === field.value)?.label ||
        ""
      );
    }
    if (field.slug.includes("thana")) {
      return (
        thanas.find((option: any) => option.value === field.value)?.label || ""
      );
    }
    if (field.slug.includes("postal_code")) {
      return (
        postal_codes.find((option: any) => option.value === field.value)
          ?.label || ""
      );
    }
    return field.value;
