...
import { FormKey } from "../types/BranchTypes";
...
const BranchAddFormModal: React.FC<UserFormModalProps> = ({
  isOpen,
  onClose,
  onBranchAdded,
}) => {
  const { form, isPending } = useAddBranch(() => {
    onBranchAdded();
    onClose();
    form.resetForm();
  });
    ...
    <div className="overflow-y-auto max-h-screen">
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.5)]">
        <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative">
          <h2 className="text-xl font-medium mb-6">Add New Branch</h2>
          ...
          <form action={form.action} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => form.update("code" as FormKey, e.target.value)}
                onBlur={() => form.onBlur("code" as FormKey)}
                required
                className="w-full border border-gray-300 rounded-lg px-4 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Enter code"
              />
              {form.shouldShowError("code" as FormKey) && (
                <p className="text-red-600 text-sm mt-1">
                  This field is required.
                </p>
              )}
...
export default BranchAddFormModal;
